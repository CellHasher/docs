...
:::info
Cellhasher purchases with phones pre-installed include ADB Authoization Keys, removing the need for this process. Proceed to [settings](../cellhasher-control/tabs-walkthrough/settings.md) to import Cellhasher-provided ADB keys, or your own.
:::
...
8. Turn off Verify Apps over USB if you need a specific app to install that is not installing or are having trouble connecting to a network using [network](../cellhasher-control/tabs-walkthrough/network.md) tab, otherwise leave off on for safety

### 3. Authorize Connection

:::note
If you intend to use a Windows/Linux PC with Cellhasher Control, you may need to have Cellhasher Control open during this step to trigger the authorization prompt. Therefore, it's recommended to complete [Set-Up (Download)](../cellhasher-control/set-up-download.md) and set the correct paths in [settings](../cellhasher-control/tabs-walkthrough/settings.md) if you do not encounter a prompt.
:::
